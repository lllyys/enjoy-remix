...
2. A GitHub issue mirroring the row (`gh` as `lllyys`). **The fork `lllyys/everyone-can-use-english` currently has Issues disabled** — if `gh issue create` fails, record `GH: n/a (issues disabled)` in the row's Notes (the mirror reminder treats that as satisfied) and treat `docs/features.md` as the sole tracker; reference work as `Refs #<row-id>` instead of a GH number.
