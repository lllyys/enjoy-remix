...
Claude Code on this machine should authenticate with a **Claude Pro/Max subscription** (interactive login / `claude setup-token`), not a raw API key. Use `ANTHROPIC_API_KEY` only in CI/headless contexts where no browser/TTY is available, and source it from a CI secret. Do not export the key in your shell profile just to suppress a login prompt — that silently routes billing to the API plan.
...
In enjoy-remix, project settings live at `.claude/settings.json` alongside the rules in `.claude/rules/` and commands in `.claude/commands/`.
...
```json
{
  ...
  "theme": "dark",
  "permissions": {
    "allow": ["Bash(yarn:*)", "Bash(git:*)", "Read", "Edit"],
    "deny": ["Bash(rm -rf:*)"]
  },
  "mcpServers": {
    "filesystem": {
...
| Variable | Purpose |
|----------|---------|
| `ANTHROPIC_API_KEY` | API authentication (CI/headless only — prefer subscription) |
...
# Process file
cat enjoy/src/main/db/models/recording.ts | claude -p "review this Sequelize model"

# Multiple files
cat enjoy/src/main/db/handlers/*.ts | claude -p "find IPC channel inconsistencies"
```
...
}' "find bugs in enjoy/src/"
```

### Budget & Turn Limits
```bash
# Spending limit (meaningful on API-billed runs)
...
claude --agents '{
  "ipc-reviewer": {
    "description": "IPC boundary expert. Use proactively for main/renderer process code.",
    "prompt": "You are an Electron expert. Focus on the main/renderer process boundary: channel-name typos, handlers that throw, bridge calls that never resolve / return undefined, non-serializable IPC payloads.",
    "tools": ["Read", "Grep", "Glob"],
    "model": "opus"
  },
  "test-writer": {
    "description": "Vitest specialist for generating comprehensive unit tests.",
    "prompt": "You write thorough Vitest unit tests for an Electron + TS app. Cover edge cases, error paths, Unicode/CJK, null/empty, and migration rollback. Tests run via `yarn workspace enjoy test:unit`.",
...
claude --agent ipc-reviewer "review the IPC handlers in enjoy/src/main/db/handlers"
```
...
enjoy-remix already ships `file-bug.md`, `file-feature.md`, and `triage.md` here. New commands follow the same shape (Markdown, optional YAML frontmatter with `description` / `argument-hint`). Example `.claude/commands/fix-issue.md`:
```markdown
---
...
Fix GitHub issue #$ARGUMENTS (gh as lllyys, repo lllyys/enjoy-remix)

Steps:
1. Fetch issue details from GitHub
2. Understand the problem
3. Locate relevant code in enjoy/src
4. Implement fix (TDD — rule 10; inner loop `yarn workspace enjoy test:unit`)
5. Write/update Vitest unit tests
6. Create commit referencing `Refs #$ARGUMENTS`
```
...
```bash
# Start in plan mode
claude --permission-mode plan "analyze the architecture of enjoy/src/main"
...
claude --allowedTools "Bash(yarn:*)" "Bash(git:*)" "Read" "Grep"
...
Bash(yarn workspace enjoy test:unit:*)   # The unit-test loop with any args
Read                     # All Read operations
Edit(enjoy/src/**:*)     # Edit files under enjoy/src/
```
...
Checks:
- Authentication status (confirms subscription vs key)
- API connectivity
- MCP server health
- Plugin status
...
### CI/CD Pipeline
API-key auth is acceptable in CI only; the key comes from a secret. The real gates the CI runs are `yarn enjoy:lint`, `yarn workspace enjoy test:unit`, and `yarn enjoy:test`.
```yaml
# GitHub Actions
- name: Claude Code Review
  ...
  run: |
    claude -p \
      ...
      --max-turns 5 \
      --max-budget-usd 2.00 \
      "Review changes for IPC, migration rollback, and Unicode/CJK issues; output JSON" > review.json
```
...
Note: the canonical pre-merge gates are the yarn commands, not the agent. Keep agent hooks advisory.
...
|-------|-------|----------|
| `Authentication failed` | Invalid/expired token | `claude setup-token` (re-auth subscription) |
...
for f in enjoy/src/main/db/handlers/*.ts; do
  claude -p "review $f" >> reviews.txt
done

# Parallel (careful with rate limits)
parallel -j2 'claude -p "review {}"' ::: enjoy/src/main/db/handlers/*.ts
```
